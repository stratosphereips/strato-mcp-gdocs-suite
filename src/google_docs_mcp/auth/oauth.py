"""OAuth 2.0 flow orchestration for Google Docs MCP."""
...
def get_credentials(
    ...
    *,
    ...
    try:
        ...
        raise DocsAuthError(f"Invalid user_id {user_id!r}: {exc}") from exc
    ...
    port = urlparse(config.redirect_uri).port or 8082
    try:
        ...
        raise DocsAuthError(f"OAuth flow failed for user {user_id!r}: {exc}") from exc
